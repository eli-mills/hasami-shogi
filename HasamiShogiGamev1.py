    ...
    def is_move_legal(self, moved_from, moving_to):
        ...
        try:
            if self.get_square_occupant(moved_from) != self.get_active_player():    # Check correct player moved.
                print("Wrong color token.")
                return False
            if self.get_square_occupant(moving_to) != "NONE":                       # Check that space is available.
                print("Space is already occupied.")
                return False
            if moved_from[0] != moving_to[0] and moved_from[1] != moving_to[1]:      # Illegal direction.
                print("Direction must be vertical or horizontal.")
                return False
            for space in self.get_game_board().get_range(moved_from, moving_to)[1:-1]:  # Check clear path.
                if space != "NONE":
                    print("Path is not clear.")
                    return False
            return True
        except IndexError:
            print("Given square is out of range.")
            return False
