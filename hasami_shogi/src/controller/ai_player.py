...
class AIPlayer(Player):
    ...
    def get_center_heuristic(self, square_string):
        """Takes a square string and returns a point value based on how close it is to the center of the board. O(1)."""
        ...
        for cap_piece in capturing_pieces:                          # O(N)
            for adj_square in get_adjacent_squares(cap_piece):      # O(1)
                if adj_square in captured_pieces:                                       # Potential capture pair found.
                    ...
                    if not active and self.find_cap_partner(game_piece_dict, cap_pair[::-1]):       # O(N)
                        continue                # In this case, one of the capturing pieces will be captured first.

                    cap_square_and_value = self.find_cap_partner(game_piece_dict, cap_pair)         # O(N)
    ...
    def find_reachable_pieces(self, game_piece_dict, square_to_reach, color_to_move):
        ...
        for piece_to_move in moving_pieces:                                         # O(N)
            path = build_square_string_range(piece_to_move, square_to_reach)        # O(N)
            if path:
                if not any([x in all_pieces for x in path[1:]]):                    # Check clear path. O(N)
    ...
    def find_capture_moves(self, game_piece_dict, captures_to_check, capturing_color):
        ...
        for square_to_reach in captures_to_check:                       # O(N)
            ...
            possible_pieces = self.find_reachable_pieces(game_piece_dict, square_to_reach, capturing_color)    # O(N^2)
    ...
    def get_capture_heuristic(self, game_piece_dict, player_turn):
        ...
        opp_pot_caps = self.find_pot_cap_squares(game_piece_dict, opponent, False)          # O(N^2)

        # Check if any potential capture squares are reachable for both players.
        active_cap_moves = self.find_capture_moves(game_piece_dict, active_pot_caps, player_turn)   # O(N^3)
        opp_cap_moves = self.find_capture_moves(game_piece_dict, opp_pot_caps, opponent)            # O(N^3)
    ...
    def get_heuristic(self, game=None):
        ...
        active_factor = factor_color_dict[active_player]["fac"]     # BLACK = 1, RED = -1
